...
integer :: n,ndat,recvd=0,failed=0,shur_fallbacks=0                              
...
double precision :: man_init_time, man_managing_time, man_waiting_time, man_record_time
double precision :: worker_compute_time=0d0
end module globals
...
logical :: ok
integer :: ierr
double precision :: wtime,total_worker_compute_time

call start_MPI(ok)
if(proc_num.eq.0) wtime=MPI_wtime()
total_worker_compute_time = 0d0
...
call MPI_Reduce(worker_compute_time,total_worker_compute_time,1, &
     MPI_DOUBLE_PRECISION,MPI_SUM,0,MPI_COMM_WORLD,ierr)
if(proc_num.eq.0) then
   wtime=MPI_wtime()-wtime
    print *,'Total time: ',wtime,' seconds'
    print *,'Total worker compute time: ',total_worker_compute_time,' seconds'
    print *,'Average worker compute time: ',total_worker_compute_time/(num_procs-1),' seconds'
    open(23,file='data/performance.csv',position='append')
    ! Write performance data to file: n, ndat, num_procs, recvd,failed, shur_fallbacks,
    ! wall time, total worker compute time, average worker compute time, manager init time, manager managing time, manager waiting time, manager record time
    write(23,'(i0,a,i0,a,i0,a,i0,a,i0,a,i0,a,f12.6,a,f12.6,a,f12.6,a,f12.6,a,f12.6,a,f12.6,a,f12.6)') &
         n,',',ndat,',',num_procs,',',recvd,',',failed,',',shur_fallbacks,',', &
         wtime,',',total_worker_compute_time,',',total_worker_compute_time/(num_procs-1),',', &
         man_init_time,',',man_managing_time,',',man_waiting_time,',',man_record_time
    close(23)
end if
call stop_MPI
