...
implicit none
logical :: conv
integer :: seed,ierr,recvd_tag,tag,SHUTDOWN_ACK_TAG
integer, dimension(mpi_status_size) :: status
double precision :: eig,compute_start
...
do while(.true.)
   ...
   worker_compute_time = worker_compute_time + MPI_Wtime() - compute_start
   ! Send result
   if(conv) then
      tag=0
   else
...
if(.not.(conv)) then
  ...
  if(info.eq.0) then
     ...
     conv=.true.
     shur_fallbacks=shur_fallbacks+1
  else
